...
class Iphonex64Spider(scrapy.Spider):
    name = "belsimpelspider"
    start_urls = [
        'https://www.belsimpel.nl/apple-iphone-x/64gb-zwart/aanbieding?abonnement=t-mobile-normaal-go-next-2-jaar-hsnwa',
        'https://www.belsimpel.nl/apple-iphone-x/64gb-zwart/aanbieding?abonnement=t2-0-minuten-5000-mb-4g-2-jaar',
        'https://www.belsimpel.nl/apple-iphone-x/64gb-zwart/aanbieding?abonnement=vodafone-red-iphonex-2-jaar',
    ...
    def parse(self, response):
        if 'belsimpel.nl' in response.request.url:
            ...
            for item in response.xpath('//*[@class="proposed-phone__row js-phone-proposition"]'):
                try:
                    if 'iphone x 64' in item.css('div.proposed-phone__image-and-name::text').extract()[1].strip().lower():
                        yield {
                            'url': response.request.url,
